...
`;class tj extends HTMLElement{#$=null;#B=null;#O=null;#H=3e3;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tP.content.cloneNode(!0))}async connectedCallback(){this.#$=this.shadowRoot?.getElementById("historyList"),this.#B=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#O=this.shadowRoot?.getElementById("testExpireBtn");let[,t=[]]=await tn(),e=(t||[]).map(t=>{if(!t)return null;if("string"==typeof t){let e=Date.now();return{value:t,addedAt:e,expiresAt:e+6048e5,notified:!1,preNotified:!1}}return{value:t.value??t?.barcode??"",addedAt:t.addedAt??Date.now(),expiresAt:t.expiresAt??(t.addedAt?t.addedAt+6048e5:Date.now()+6048e5),notified:!!t.notified,preNotified:!!t.preNotified}}).filter(Boolean);await tl(e),this.#P(e||[]),this.#$?.addEventListener("click",this.#j),this.#B?.addEventListener("click",this.#F),this.#O?.addEventListener("click",this.#V),this.#U()}disconnectedCallback(){this.#$?.removeEventListener("click",this.#j),this.#B?.removeEventListener("click",this.#F),this.#O?.removeEventListener("click",this.#V),this.#W()}async add(t){if(!t)return;let e={type:"add",message:"Error adding barcode to history"},[o,i=[]]=await tn();if(o||!Array.isArray(i))return this.#G("bs-history-error",e),o;let r=(i||[]).map(t=>"string"==typeof t?{value:t,addedAt:Date.now(),expiresAt:Date.now()+6048e5,notified:!1,preNotified:!1}:{value:t.value??t.barcode??"",addedAt:t.addedAt??Date.now(),expiresAt:t.expiresAt??(t.addedAt?t.addedAt+6048e5:Date.now()+6048e5),notified:!!t.notified,preNotified:!!t.preNotified}),s="string"==typeof t?t:t.value;if(r.find(t=>t.value===s))return;let a=Date.now(),n={value:s,addedAt:a,expiresAt:a+6048e5,notified:!1,preNotified:!1},l=[...r,n],[d]=await tl(l);return d?(tc.error("Error setting history",d),this.#G("bs-history-error",e),d):(this.#$?.insertBefore(this.#Y(n),this.#$.firstElementChild),this.#G("bs-history-success",{type:"add",message:"Barcode added to history"}),this.#Z(),null)}async remove(t){if(!t)return;let e={type:"remove",message:"Error removing barcode from history"},[o,i=[]]=await tn();if(o||!Array.isArray(i))return this.#G("bs-history-error",e),o;let r=(i||[]).filter(e=>"string"==typeof e?e!==t:e.value!==t),[s]=await tl(r);if(s)return tc.error("Error setting history",s),this.#G("bs-history-error",e),s;let a=this.#$?.querySelector(`li[data-value="${t}"]`);return a?.remove(),this.#G("bs-history-success",{type:"remove",message:"Barcode removed from history"}),this.#Z(),null}async empty(){let[t]=await tl([]);return t?(tc.error("Error setting history",t),this.#G("bs-history-error",{type:"empty",message:"Error emptying history"}),t):(this.#$?.replaceChildren(),this.#G("bs-history-success",{type:"empty",message:"History emptied successfully"}),this.#Z(),null)}#V=async()=>{try{let t=`TEST-EXP-${Date.now()}`,[e,o=[]]=await tn();if(e||!Array.isArray(o))return void this.#G("bs-history-error",{type:"add",message:"Error adding test item to history"});let i=(o||[]).map(t=>"string"==typeof t?{value:t,addedAt:Date.now(),expiresAt:Date.now()+6048e5,notified:!1,preNotified:!1}:{value:t.value??t.barcode??"",addedAt:t.addedAt??Date.now(),expiresAt:t.expiresAt??(t.addedAt?t.addedAt+6048e5:Date.now()+6048e5),notified:!!t.notified,preNotified:!!t.preNotified}),r=Date.now(),s={value:t,addedAt:r,expiresAt:r+5e3,notified:!1,preNotified:!1},a=[s,...i],[n]=await tl(a);if(n)return void this.#G("bs-history-error",{type:"add",message:"Error adding test item to history"});this.#$?.insertBefore(this.#Y(s),this.#$.firstElementChild),this.#G("bs-history-success",{type:"add",message:"Test item added to history"}),this.#Z()}catch(t){}};#P(t){if(!this.#$)return;this.#$.replaceChildren();let e=document.createDocumentFragment();[...t].reverse().forEach(t=>e.appendChild(this.#Y(t))),this.#$.appendChild(e)}#Y(t){let e,o=document.createElement("li"),i="string"==typeof t?t:t.value||"",r=t?.expiresAt||Date.now()+6048e5;o.setAttribute("data-value",i);try{new URL(i),(e=document.createElement("a")).href=i,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=i;let s=document.createElement("span");s.className="history-countdown",s.setAttribute("aria-live","polite"),s.dataset.expiresAt=String(r),s.textContent=this.#X(r-Date.now());let a=document.createElement("div");a.className="actions";let n=document.createElement("custom-clipboard-copy"),l=n.shadowRoot?.querySelector("button");n.setAttribute("only-icon",""),n.setAttribute("value",i),l?.setAttribute("aria-label",`Copy to clipboard ${i}`),a.appendChild(n);let d=document.createElement("button");return d.type="button",d.className="delete-action",d.setAttribute("data-action","delete"),d.setAttribute("aria-label",`Remove from history ${i}`),d.innerHTML=`
    ...
    `,a.appendChild(d),o.appendChild(e),o.appendChild(s),o.appendChild(a),o}#j=async t=>{let e=t.target;if(e.closest('[data-action="delete"]')){let t=e.closest("li").dataset.value;window.confirm(`Delete history item ${t}?`)&&this.remove(t)}};#J=null;#U(){this.#J||(this.#J=setInterval(()=>this.#Z(),1e3),this.#Z())}#W(){this.#J&&(clearInterval(this.#J),this.#J=null)}async #Z(){try{let[,t=[]]=await tn(),e=(t||[]).map(t=>"string"==typeof t?{value:t,addedAt:Date.now(),expiresAt:Date.now()+6048e5,notified:!1,preNotified:!1}:{value:t.value??t.barcode??"",addedAt:t.addedAt??Date.now(),expiresAt:t.expiresAt??(t.addedAt?t.addedAt+6048e5:Date.now()+6048e5),notified:!!t.notified,preNotified:!!t.preNotified}),o=Date.now();(this.shadowRoot?.querySelectorAll(".history-countdown")||[]).forEach(t=>{let e=Number(t.dataset.expiresAt)||o;t.textContent=this.#X(e-o)});let i=!1;for(let t of e){if(!t)continue;let e=(t.expiresAt||0)-o;e>0&&e<=this.#H&&!t.preNotified&&(this.#K(t,e),t.preNotified=!0,i=!0),e<=0&&!t.notified&&(this.#Q(t),t.notified=!0,i=!0)}i&&await tl(e)}catch(t){}}async #K(t,e){try{let o=`${t.value} will expire in ${this.#X(e)}.`;try{tA(o,{variant:"warning"})}catch(t){}"Notification"in window&&("default"===Notification.permission&&await Notification.requestPermission(),"granted"===Notification.permission&&new Notification("Item will expire soon",{body:o}))}catch(t){}}#X(t){if(t<=0)return"Expired";let e=Math.floor(t/1e3),o=Math.floor(e/86400);if(o>0)return`${o} day${1===o?"":"s"} left`;let i=Math.floor(e%86400/3600);if(i>0)return`${i} hour${1===i?"":"s"} left`;let r=Math.floor(e%3600/60);return r>0?`${r} min left`:"Less than a minute"}async #Q(t){try{let e=`${t.value} may have reached its expiry date.`;try{tA(e,{variant:"warning"})}catch(t){}"Notification"in window&&("default"===Notification.permission&&await Notification.requestPermission(),"granted"===Notification.permission&&new Notification("Item may have expired",{body:e}))}catch(t){}}#F=async()=>{window.confirm("Empty history? This action cannot be undone.")&&this.empty()};#G(t,e=null){let o=new CustomEvent(t,{bubbles:!0,composed:!0,detail:e});this.dispatchEvent(o)}static defineCustomElement(t="bs-history"){"undefined"==typeof window||window.customElements.get(t)||window.customElements.define(t,tj)}}tj.defineCustomElement(),async function(){let t=document.querySelector("a-tab-group"),e=document.querySelector("video-capture"),o=document.querySelector("bs-settings"),i=document.querySelector("bs-history"),r=document.getElementById("cameraPanel"),s=r.querySelector(".results"),a=document.getElementById("filePanel"),n=a.querySelector(".results"),l=document.getElementById("scanInstructions"),d=document.getElementById("scanBtn"),c=document.getElementById("dropzone"),h=document.querySelector("resize-observer"),u=document.getElementById("scanFrame"),m=document.getElementById("torchButton"),p=document.getElementById("globalActions"),b=document.getElementById("historyBtn"),g=document.getElementById("historyDialog"),f=document.getElementById("settingsBtn"),v=document.getElementById("settingsDialog"),y=document.getElementById("settingsForm"),w=document.getElementById("cameraSelect"),E=null,A=!0;"function"==typeof HTMLDialogElement&&(p?.removeAttribute("hidden"),g?.removeAttribute("hidden"),v?.removeAttribute("hidden"));let{barcodeReaderError:x}=await tv.setup();if(x){let e=document.getElementById("barcodeReaderError");A=!1,p?.setAttribute("hidden",""),t?.setAttribute("hidden",""),e?.setAttribute("open","");return}let C=await tv.getSupportedFormats(),[,k]=await ts(),L=k?.formats||C,_=await tv.create(L);async function S(t,e){try{let o=await tw(t);if(o){let i=o.title||o.name||o.alias||"",r=o.description||o.brand||"",s=`${i||t}${r?` \u{2014} ${r}`:""}`;tA(s,{variant:"success"}),function(t,e){if(!t||!e)return;let o=t.querySelector("#itemInfo");if(!o){(o=document.createElement("div")).id="itemInfo",o.className="item-info";let e=t.querySelector(".results");e&&e.parentNode?e.parentNode.insertBefore(o,e.nextSibling):t.appendChild(o)}o.textContent="";let i=document.createElement("h3");i.className="item-info__title",i.textContent=e.title||e.name||e.alias||"";let r=document.createElement("p");r.className="item-info__description",r.textContent=e.description||"";let s=document.createElement("p");s.className="item-info__brand",s.textContent=e.brand?`Brand: ${e.brand}`:"",o.appendChild(i),r.textContent&&o.appendChild(r),s.textContent&&o.appendChild(s)}(e,o);try{let i=e?.querySelector(".results");if(i){let e=i.querySelector(`bs-result[value="${t}"]`);e&&(o.title&&e.setAttribute("data-title",o.title),o.brand&&e.setAttribute("data-brand",o.brand),o.description&&e.setAttribute("data-description",o.description))}}catch(t){}}}catch(t){}}e.addEventListener("video-capture:video-play",q,{once:!0}),e.addEventListener("video-capture:error",function(t){let e=t.detail.error;if("NotFoundError"===e.name)return;let o="NotAllowedError"===e.name?"<strong>Error accessing camera</strong><br>Permission to use webcam was denied or video Autoplay is disabled. Reload the page to give appropriate permissions to webcam.":e.message;r.innerHTML=`
